...
public class HeaderController {
    ...
    private String consumerKey, consumerSecret, signatureMethod, oauthToken, oauthTokenSecret, version;


    public HeaderController(String consumerKey, String consumerSecret, String oauthToken, String oauthTokenSecret) {
    ...
    public String generateHeader(String httpMethod, String url) {
        ...
        String baseSignatureString = generateSignatureBaseString(httpMethod, url, nonce, timestamp);
        String signature = encryptUsingHmacSHA1(baseSignatureString);
        base.append("OAuth ");
        append(base, oauth_consumer_key, consumerKey);
        append(base, oauth_token, oauthToken);
        ...
        base.deleteCharAt(base.length() - 1); // delete ','
    ...
    private String generateSignatureBaseString(String httpMethod, String url, String nonce, String timestamp) {
        ...
        put(params, oauth_token, oauthToken);
        ...
        base.deleteCharAt(base.length() - 1); // delete last '&'
    ...
    private String encryptUsingHmacSHA1(String input) {
        String secret = new StringBuilder().append(encode(consumerSecret)).append("&").append(encode(oauthTokenSecret)).toString();
    ...
    private String encode(String value) {
        String target = "";
        try {
            target = URLEncoder.encode(value, "UTF-8");
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        for( int i=0; i < target.length(); i++) {
            focus = target.charAt(i);
            if (focus == '*') {
                convertedString += "%2A";
            } else if (focus == '+') {
                convertedString += "%20";
            } else if (focus == '%' && i + 1 < target.length() && target.charAt(i + 1) == '7' && target.charAt(i + 2) == 'E') {
                convertedString += '~';
                i += 2;
            } else {
                convertedString += focus;
            }
        }
    ...
    private String getNonce() {
        int left = 48; // number '0'
        int right = 122; // 'z'
        int stringLength = 10;
        Random random = new Random();

        String generatedString = random.ints(left, right+1).filter(i -> (i <= 57 || i >= 65) && (i <= 90 || i >= 97)).limit(stringLength)
